...
use crate::{
    commands::helpers::{
        file_type::{get_file_type, FileType},
...
pub async fn ocr(
    ...
    let key = {
        ...
        match key {
            ...
            Some(key) => String::from_utf8(key).map_err(|err| {
    ...
    file_paths.iter().for_each(|file_path| {
        ...
        match path {
            Err(err) => {
                ...
                app.emit("error_ocr", format!("file not accessible: {}", file_path.1))
                    .ok();
            }
            Ok(path) => match get_file_type(&path) {
                Err(err) => {
                    eprintln!("Error getting file type: {}", err);
                    app.emit(
                        "error_ocr",
                        format!("error gettong file typr: {:?}", path.to_owned()),
                    )
                    .ok();
                }
                Ok(file_type) => {
                    match file_type {
                                    ...
                                    match img.save_with_format(&webp_path, image::ImageFormat::WebP)
                                    {
                                        Err(err) => {
                                            ...
                                            app.emit(
                                                "error_ocr",
                                                format!(
                                                    "Error getting pdf from: {:?}",
                                                    file_path.1.to_owned()
                                                ),
                                            )
                                            .ok();
                                            return;
                        ...
                        FileType::PDF => {
                            ...
                            let pages = match pdf.render(
                                pdf2image::Pages::Range(0..=pdf.page_count()),
                                RenderOptionsBuilder::default()
                                    .build()
                                    .expect("Error getting default renfer options"),
                            ) {
                                Err(err) => {
    ...
    'requester: loop {
        let input_vec: Vec<Input> = (0..5)
            .filter_map(|_| match webp_queue.pop_front() {
                None => return None,
